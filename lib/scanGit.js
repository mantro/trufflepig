  ...
  history.on("commit", async function (commit) {
    ...
    for (let diff of diffList) {
      ...
      for (let patch of patches) {
        ...
        let ignored = options.ignoreGlobs.find(f => minimatch(patch.newFile().path(), f));
        if (ignored) {
          return;
        }
        ...
        for (let hunk of hunks) {
          ...
          for(let x = 0; x < lines.length; x++) {
            ...
            if (output.length > 0) {
              console.log();
              console.log(colors.yellow('###> ' + summary.date + ' ' + summary.author + ' ' + summary.commit));
              console.log(colors.yellow('###> ' + summary.newFile + ', Line ' + (x+1)));
