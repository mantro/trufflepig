    ...
    for (let diff of diffList) {
      ...
      for (let patch of patches) {
        ...
        for (let hunk of hunks) {
          ...
          lines.forEach((line) => {
            for (let str of stringsOfSet(line.content().trim(), options.BASE64_CHARS, options.wordLength)) {
            ...
            for (let str of stringsOfSet(line.content().trim(), options.HEX_CHARS, 12)) {
