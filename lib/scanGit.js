            ...
            if (result.length > 0) {
              ...
              for (let resultLine of result) {
                ...
                if (options.showDuplicates || hashmap.addIfNotExists(resultLine.output)) {
                  ...
                  winston.info(colors.grey('date'), colors.green(summary.date), colors.grey('author'), colors.green(summary.author), colors.grey('sha1'), colors.green(summary.commit));
                  winston.info(colors.grey('file'), colors.green(`${summary.newFile}:${x + 1}`), colors.grey('entropy'), colors.green(`${resultLine.entropy} (${resultLine.prefix})`));
